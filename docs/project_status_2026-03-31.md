...
경고 화면에서 본인이 상황을 판단한 뒤 "안전 확인 — 위험하지 않습니다" 버튼을 누르면,
현재 위험 세션이 즉시 종료됩니다. 오탐(false positive)일 때 불필요한 경고를 빠르게 해제할 수 있습니다.

### 보호자 관리

신뢰할 수 있는 가족이나 지인을 최대 3명까지 보호자로 등록할 수 있습니다.
위험 상황에서 보호자에게 바로 전화를 걸 수 있고, 문자 메시지를 직접 보낼 수도 있어 혼자 판단하기 어려울 때 도움을 받을 수 있습니다.
(문자는 자동 발송이 아니라, 본인이 직접 발송 버튼을 누릅니다.)
...
**70개 단위 테스트** 작성 완료, **전체 통과** 확인되었습니다.

| 검증 영역 | 테스트 수 | 확인하는 내용 |
|-----------|:---------:|--------------|
| 위험 점수 계산 | 18개 | 10개 신호 가중치, 위험 수준 경계, P0.5 신호 점수, 복합 패턴 CRITICAL 강제 상향 |
| 은행 ARS 번호 매칭 | 15개 | 8개 은행 번호 정확 매칭, 하이픈/공백 정규화, 비매칭 검증 |
| 통화 신호 매핑 | 12개 | 미저장/신규/검증 번호 분류, 엣지 케이스 |
| 위험 세션 관리 | 11개 | 신호 누적, 세션 시작/유지/종료, 텔레뱅킹 시나리오, 알림 diff 감지 |
| 경고 화면 | 7개 | 보호자 정보, 위험 이벤트 표시, 안전 확인 세션 종료 |
| 위험 이벤트 생성 | 7개 | 각 상황에 맞는 경고 메시지 생성 |
...
| 2026-03-31 | `9f285c8` | **P5**: 현황 보고서 v2.0 반영 |
| 2026-04-01 | `18cb101` | 미사용 테스트 제거 — bankingInterrupterShown 정리 |
| 2026-04-01 | `7df87d3` | 권한 화면 D-pad 토글 수정 + 테스트 모드 10초 임계값 |
| 2026-04-01 | `2ed5b95` | 보호자 수동 SMS 메뉴 추가 (ACTION_SENDTO) |
| 2026-04-01 | `1fb9e97` | 안전 확인 버튼 — 경고 화면에서 세션 종료 경로 |
| 2026-04-01 | `5c4fca0` | 리뷰 반영 — 오버레이 갱신, 디버그 트리거, 스레드 안전성, 문서 |
| 2026-04-01 | `477c669` | 텔레뱅킹 시뮬레이션 버튼 + PermissionType enum 리팩터링 |
| 2026-04-01 | `e9ac60c` | 단위 테스트 25개 추가 — P0.5 신호, 복합 CRITICAL, BankArsRegistry |
...
| 4 | 세션 타임아웃 테스트 | Clock 주입 리팩터링 후 30분 타임아웃 경로 검증 |
