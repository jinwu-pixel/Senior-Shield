# Copy of investigations/2026-09-05-integration-m3-permission/run-device-verification.ps1 (34832de) for the
# post-merge main baseline. Only two edits: evidence is written into THIS track folder, and the
# instrumentation validator is referenced from the original track. Preflight, grant/revoke, and
# finally-cleanup are unchanged.
...
function Run-Instrumentation([string]$Phase,[int]$Expected,[bool]$SmokeOnly) {
    ...
    & "$root/investigations/2026-09-05-integration-m3-permission/verify-instrumentation.ps1" -Report (Join-Path $evidence "$Phase-instrumentation.txt") -SmokeOnly:$SmokeOnly
