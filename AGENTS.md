# AGENTS.md — 시니어쉴드 프로젝트 지침

이 파일은 Codex가 세션 시작 시 자동으로 읽는 프로젝트 지침이다.
...
---
# AGENTS.md 추가 지침 v2 (2026-03-31 리뷰 반영)

기존 AGENTS.md의 "고위험 작업" 섹션 뒤에 삽입한다.
...
## 협업 규약 — 리스크 등급 파이프라인 (2026-07-21 확정)

역할: 계획·검토 = Claude / 수행 = Codex. DIRECTIVE 기반 트랙 운영.
이 규약은 Claude memory `feedback_collaboration_protocol.md`와 동기(양측 세션 간 지속).

### 등급 (Claude가 DIRECTIVE 첫 줄에 명시)
- **T0 문서·보고**: 앱 동작·테스트 무변.
- **T1 격리 코드**: 기존 seam, **동시성·공유가변상태·interface·정책 표면 없음**, 작은 blast radius.
- **T2 고위험**: 위 "고위험 작업" 목록 전부 + 동시성/멀티플로우/monitor/safe-confirm/interface.
- **애매하면 위로.** 실행 중 부족 판명 = 중단·질의(S3).

### 게이트 (검토 깊이 = 등급 비례)
- T0: 자기교정. **T1: raw 핸드오프(자체검토 생략)** — Claude 독립검토가 유일 게이트.
  T2: 핸드오프 전 전체 자체검토(+Claude 적대 워크플로).

### 정지·질의 = S1~S5만 (그 밖 자동 전진, 한 줄 로그 `✓ <단위> [T?] — <수치>`)
- **S1** 되돌릴 수 없는 게이트: commit·push·실기기 조작·Manifest/권한/DI 실제 변경.
- **S2** 지적 ≥recommend. **S3** 등급 상승. **S4** 제품·정책 결정. **S5** 완료 게이트 실패(RED·lint 신규·범위 위반).

### 성문화
- DIRECTIVE 템플릿: `investigations/_DIRECTIVE_TEMPLATE.md` 헤더로 시작(등급/검토/S1~S5).
- 진행 중 트랙은 기존 계약(예: 실기 게이트 v1.1)으로 마무리하고, 신규 트랙부터 이 규약 적용.
